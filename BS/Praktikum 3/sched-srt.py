...
def get_runtime(pid):	    return   sum(tasks[pid]["behavior"][0:-1:2])
...
def schedule():
  # Implementierung des SJF (Shortest Job First)
  ...
  if runqueue + blocked + futureprocesses(cputime) == []:
    print "ENDE"
    choice = -2
  # falls nicht: nehme kürzeste verbleibene Rechenzeit aus Runqueue
  elif (runqueue != []):
    current = runqueue[0]
    print "suche nach kürzester"
    tmp = get_runtime(current)
    #print tmp
    print "current: ", current, " tmp=", tmp
    for item in runqueue:
      print "item: ", item, " head_behavior: ", get_head_behavior(item)
      if (get_head_behavior(item) < tmp):
	print get_head_behavior(item), " ist kuerzer als ", tmp
        tmp = get_head_behavior(item)
	current = item
    choice = current
    print "choice ", choice
  else:
